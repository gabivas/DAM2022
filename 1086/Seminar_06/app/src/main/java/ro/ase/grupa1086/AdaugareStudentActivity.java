...
import static ro.ase.grupa1086.MainActivity.EDIT_STUDENT;
...
public class AdaugareStudentActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(intent.hasExtra(EDIT_STUDENT)){
            Student student = (Student)intent.getSerializableExtra(EDIT_STUDENT);
            etNume.setText(student.getNume());
            etDataInmatriculare.setText(new SimpleDateFormat(DATE_FORMAT,Locale.US)
                    .format(student.getDataInmatriculare()));
            etVarsta.setText(""+student.getVarsta());
            ArrayAdapter<String> adapter1= (ArrayAdapter<String>) spinner.getAdapter();
            for(int i=0;i<adapter1.getCount();i++){
                if (student.getFacultate().toString()
                        .equals(adapter1.getItem(i))) {
                    spinner.setSelection(i);
                    break;
                }
            }
            if(student.getFormaInvatamant().toString()
                    .equals("CU_FRECVENTA")){
